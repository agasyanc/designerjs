...
class HarfBuzzExports {
  ...
  readonly hbjs_glyph_svg: (facePtr: Pointer, glyph: number, pathBuffer:number, pathBufferSize:number) => Pointer
  readonly hb_font_set_variations: (fontPtr: Pointer, variations: any, numVariations: number) => void
  ...
  constructor(exports: any) {
    ...
    this.hb_font_set_variations = exports.hb_font_set_variations;
...
export class HarfBuzzFont {
  ...
  shape(text: string): Array<GlyphInformation> {
  ...
  glyphToPath(glyphId:number){
    let pathBufferSize = 65536;
    let pathBuffer = hb.malloc(pathBufferSize);
    
    let svgLength = hb.hbjs_glyph_svg(this.ptr, glyphId, pathBuffer, pathBufferSize);
    
    var utf8Decoder = new TextDecoder("utf8");
    return svgLength > 0 ? utf8Decoder.decode(hb.heapu8.subarray(pathBuffer, pathBuffer + svgLength)) : "";
  }
...
export class HarfBuzzBuffer {
  ...
  shape(font: HarfBuzzFont, features: any) {
    let featuresPtr = hb.malloc(features.length * 4);
    hb.heapu32.set(features, featuresPtr / 4);
    hb.hb_shape(font.ptr, this.ptr, featuresPtr, features.length);
    hb.free(featuresPtr);
